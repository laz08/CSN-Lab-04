...
To have a general vision on how these community algorithms work in practice, we are going to use them on 4 different networks:
...
- A graph made with *3 full-connected graphs*, connected with by 2 edges.
...
```{r  echo=FALSE}
plot.igraph(karate,layout=layout.auto,vertex.size=23,vertex.label.color="yellow",vertex.shape = "sphere", vertex.label.font=2,vertex.color="darkblue",edge.color="black")
```
