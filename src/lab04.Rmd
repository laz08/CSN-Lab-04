...
(All the information can be found either in the slides or https://stackoverflow.com/questions/9471906/what-are-the-differences-between-community-detection-algorithms-in-igraph or https://www.r-bloggers.com/summary-of-community-detection-algorithms-in-igraph-0-6/ and respective pointed papers.)

# 2. Metrics

We have calculates the metrics following these definitions:
...
*  *modularity*: difference between the number of edges in C and the expected number of edges E[mc] of a random graph with the same degree distribution. As we cannot suppose the type of data we are going to get, and so we cannot infer the type of graph, we calculate the expected number of edges in a simple way, being: 

TODO: find expression for expected edges!


After computing the metrics for each community, we use an weighted average to calculate the global values for the whole network. This being:

\[ 
metric(G) = \sum_{C \in comm(G)} \frac{n_C}{n}*metric(C)
\],
where $n_C$ is the number of nodes of C and $n$ is the number of total nodes of the network.
...
by us for its high computational demanding due to its inner working methods of computing the betweenesses, removing the edge with the highest betweeness, recomputing the betweenesses, then removed edge and removing another one, compute again... And so on. (from https://arxiv.org/pdf/cond-mat/0112110.pdf)
