...
kable(karateMetricsTable,
      ...
      col.names = c("Method", "$\\Delta$ time", "# $C$", "TPT", "Expansion", "Conductance", "Modularity"))
```
...
Generally, in all 5 networks, expansion is low except in Coxeter and Zachary. This means that not many edges per node are leaving the community detected by the algorithm, which is good. In case of Coxeter and Zachary, it seems that the communities found have nodes that are well-connected with others from different communities. Leading to a higher value.

As for Conductance, it is kept pretty low in all networks (between 0.1 and 0.2), which means that the fraction of volum edges pointing outside the cluster is low.  It increases a little (0.4) in all methods in the case of Coxeter. Again, Coxeter seems to be pretty intertwined when creating the communities as seen in the figure above, so this does not surprise us.

Finally, if we check modularity, it ranges from 0.3 to 0.6 in general. Their highest values are found on the three full connected graphs and the modified Thomassen. 
It seems then that the difference between the expected number of edges of a random graph with the same distribution and our models' number of edges is greater.
...
by us for it is high computational demanding due to its inner working methods of computing the betweenesses, removing the edge with the highest betweeness, recomputing the betweenesses, then restoring the edge and removing another one, compute again...
\footnote{https://arxiv.org/pdf/cond-mat/0112110.pdf}. 
...
We have tried to plot the whole graph but of course nothing can really be determined by looking at it, because it is very crowded.

Because of this we are only going to take a look some communities.
The first two, the tenth, the 501st and two of the last ones.

```{r   out.width="350px", out.height="350px", fig.align='center'}
vec = c(1, 2, 10, 501, 3300, 3352)
plotGraphSetOfCommunities(walktrapCommsWiki, wikiG, vec)
```

We can see that the first ones are well-populated and the last communities do not have many nodes (either 2 or 1). Thus, it seems that the last communities found are articles unrelated to others.

If we take a look at the first community, it seems that its center is Egyptology and the topics revolve around that articles. The articles shown are related to the topic so we think is community is pretty well-defined.

Community 2 seems to have taken the randomized factor into account. On the upper-center we have the article *falling in love* and at the bottom-left we have *Image Constraint Token*. In the middle, their path contains:

**Image Constraint Token** $\rightarrow$ Advanced Access Content System $\rightarrow$ Analog Hole Multimedia $\rightarrow$ Vulnerability $\leftarrow$ **Falling in love**. 

So, since it is a directed graph, they are mainly connected by the Vulnerability topic.


The tenth and 501st communities seems to have a central topic as the first one, which are the community topics and biometrics articles, respectively.




# 3. Final brief summary

Spinglass has proven to be the slowest method for creating communities.
...
Coxeter is an interesting network to work with. Their TPT is 0 because creating communities splits the existing TPT. It has been the exception in all metrics.

Thomassen and three full connected graphs are the graphs with higher modularity. Probably because we forced the full connected graphs, which is greater than the expected value E[$m_{c}$]
